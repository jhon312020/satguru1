...
class Supplier_Model extends CI_Model
{
	...
	function insert_house_info($atimefrom,$dtimebefore,$checkin_from1,$costin1,$checkin_from2,$costin2,$checkout_from1,$costout1,$checkout_from2,$costout2,$mistay,$mxstay,$rentamt,$pmode,$rentamtday,$clean,$supp,$apt_id,$policy,$pets)
	{
		$data = array('sup_apart_list_id'=>$apt_id,'arrivaltime_from'=>$atimefrom,'departtime_before'=>$dtimebefore,'checkin_time1'=>$checkin_from1,'checkin_time2'=>$checkin_from2,'checkout_time1'=>$checkout_from1,'checkout_time2'=>$checkout_from2,'checkin_extracost1'=>$costin1,'checkin_extracost2'=>$costin2,'checkout_extracost2'=>$costout1,'checkout_extracost1'=>$costout2,'mini_stay'=>$mistay,'max_stay'=>$mxstay,'rent_amount'=>$rentamt,'rent_amount_days'=>$rentamtday,'payment_mode'=>$pmode,'cleaning'=>$clean,'supplies'=>$supp,'policy'=>$policy,'pets'=>$pets);
		$this->db->insert('sup_apart_houserules',$data);
	}
	function update_house_info($atimefrom,$dtimebefore,$checkin_from1,$costin1,$checkin_from2,$costin2,$checkout_from1,$costout1,$checkout_from2,$costout2,$mistay,$mxstay,$rentamt,$pmode,$rentamtday,$clean,$supp,$apt_id,$policy,$pets)
	{
	...
	function add_rate_details($rate_plan,$default_avail,$capacity,$def_max,$def_min,$rate,$booking,$breakfast,$breakfast_type,$breakfastfrom,$breakfastto,$cancellation,$cancellation_days,$cancellation_nights,$charges,$charge_percent,$charge_rate,$per_night,$booking_details,$cancel_details,$apt_id,$plan,$newdate,$newdate1,$remarks_room,$diff)
	{
		//$this->db->where('sup_apart_list_id',$apt_id);
	//	$this->db->delete('sup_apart_rateplan');
		
		$data = array('sup_apart_list_id'=>$apt_id,'rate_name'=>$rate_plan,'default_availablity'=>$default_avail,'capacity'=>$capacity,'max_stay'=>$def_max,'mini_stay'=>$def_min,'default_rate'=>$rate,'booking_type'=>$booking,'breakfast'=>$breakfast,'breakfast_type'=>$breakfast_type,'breakfast_from'=>$breakfastfrom,'breakfast_to'=>$breakfastto,'book_policy'=>$booking_details,
		'cancel_comments'=>$cancel_details,'policy_flag'=>$cancellation,'hours1'=>$cancellation_days,'hours2'=>$cancellation_nights,'charges'=>$charges,'charge_persent'=>$charge_percent,'charge_price'=>$charge_rate,'charge_nights'=>$per_night,'sup_apart_category_id'=>$plan,'startdate'=>$newdate,'enddate'=>$newdate1,'rate_remarks'=>$remarks_room);
		...
		for ($i=0; $i<=$diff; $i++)
			{
				...
				$data1 = array('sup_apart_rateplan_id'=>$latest_id,'sup_apart_list_id'=>$apt_id,'date'=>$date,'day'=>$new1,'available'=>$default_avail,'min_stay'=>$def_min,'maxi_stay'=>$def_max,'rate'=>$rate,'month'=>$month,'year'=>$year,'capacity'=>$capacity);
				$this->db->insert('sup_apart_maintain_month',$data1);
			}
		

	}
	function update_rate_details($rate_plan,$default_avail,$capacity,$def_max,$def_min,$rate,$booking,$breakfast,$breakfast_type,$breakfastfrom,$breakfastto,$cancellation,$cancellation_days,$cancellation_nights,$charges,$charge_percent,$charge_rate,$per_night,$booking_details,$cancel_details,$apt_id,$id,$newdate,$newdate1,$remarks_room,$diff)
	{
		//echo $id;exit;
	//echo $cancellation_days;echo $charge_percent;exit;
		...
		//$this->db->delete('sup_apart_rateplan');
		$data = array('sup_apart_list_id'=>$apt_id,'rate_name'=>$rate_plan,'default_availablity'=>$default_avail,'capacity'=>$capacity,'max_stay'=>$def_max,'mini_stay'=>$def_min,'default_rate'=>$rate,'booking_type'=>$booking,'breakfast'=>$breakfast,'breakfast_type'=>$breakfast_type,'breakfast_from'=>$breakfastfrom,'breakfast_to'=>$breakfastto,'book_policy'=>$booking_details,
		'cancel_comments'=>$cancel_details,'policy_flag'=>$cancellation,'hours1'=>$cancellation_days,'hours2'=>$cancellation_nights,'charges'=>$charges,'charge_persent'=>$charge_percent,'charge_price'=>$charge_rate,'charge_nights'=>$per_night,'startdate'=>$newdate,'enddate'=>$newdate1,'rate_remarks'=>$remarks_room);
		$this->db->where('sup_apart_rateplan_id',$id);
		...
		for ($i=0; $i<=$diff; $i++)
			{
				...
				if($query->num_rows() =='')
				{
					 $data1 = array('sup_apart_rateplan_id'=>$id,'sup_apart_list_id'=>$apt_id,'date'=>$date,'day'=>$new1,'available'=>$default_avail,'min_stay'=>$def_min,'maxi_stay'=>$def_max,'rate'=>$rate,'month'=>$month,'year'=>$year,'capacity'=>$capacity);
				$this->db->insert('sup_apart_maintain_month',$data1);
				}
				else
				{
					 ...
					 $sup_apart_maintain_month_id = $val->sup_apart_maintain_month_id;
		  			$data2 = array('sup_apart_list_id'=>$apt_id,'date'=>$date,'day'=>$new1,'available'=>$default_avail,'min_stay'=>$def_min,'maxi_stay'=>$def_max,'rate'=>$rate,'month'=>$month,'year'=>$year,'capacity'=>$capacity);
   ...
   function get_apartfecilities($id)
	...
	function add_check_pictures($apartid,$checkval,$comments)
	{
		$data = array('title'=>$comments,'status'=>1);
	...
	{
		$data = array('sup_apart_list_id'=>$id,'image_name'=>$img);
		$this->db->insert('sup_apart_pictures',$data);
	}
	function upload_room_picture($img,$id,$room_id)
	{
		$data = array('sup_apart_list_id'=>$id,'image_name'=>$img,'sup_apart_category_id'=>$room_id);
...
//		echo $id;exit;
	...
	function insert_sup_list($fname,$lname,$user_id,$apt_name,$city,$country,$language,$email,$apt)
	{
		$data = array('first_name'=>$fname,'last_name'=>$lname,'user_id'=>$user_id,'apartment_name'=>$apt_name,'city'=>$city,'country_id'=>$country,'language_id'=>$language,'email'=>$email,'apartment_id'=>$apt);
