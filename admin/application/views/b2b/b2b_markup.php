			...
			<div class="container-fluid">
				...
				<div class="box box-color box-bordered red">
							<div class="box-title">
								<h3>
									<i class="icon-ok"></i>
									B2B Markup - <?php echo $ty; ?>
								</h3>
							</div>
							<div class="box-content">
							<h4>API BASED</h4>
								<form action="<?php echo WEB_URL_ADMIN ?>b2b/b2bmarkupinsert/1" method="POST" class='form-horizontal form-validate' id="bb">
                                <div class="control-group">
										<label for="textfield" class="control-label">Agent Name</label>
										<div class="controls">
											<select name="agent" id="agent" class="input-xlarge" data-rule-required="true">
                                            
											<?php foreach($agent as $key => $value) { ?>
												<option value="<?php echo $value->agent_id?>"><?php echo $value->name ?> | <?php echo $value->company_name ?></option>
											<?php } ?>
											</select>
										</div>
									</div>
							...
							<div class="box-content nopadding">
								<table class="table table-hover table-nomargin table-bordered dataTable-columnfilter dataTable">
									...
									<tbody>
										<?php $i =1; if(isset($b2b_markup_g)) { if($b2b_markup_g != '') { foreach($b2b_markup_g as $key => $list) { ?>
										<tr>
											<td><?php echo $i; ?></td>
											<td><?php echo $list->api; ?></td>
                                            	<td><?php echo $list->agent_id; ?></td>
											<td class='hidden-350'><?php if($list->markup_type == 1); echo 'Hotel'; ?></td>
                                            <td class='hidden-1024'>All</td>
											<td class='hidden-1024'><?php echo $list->markup; ?>%</td>
											
											<td class='hidden-480'>
												<a href="<?php echo WEB_URL_ADMIN; ?>b2b/delete_markup/<?php echo $list->id.'/'.$list->markup_type; ?>">Delete</a>
											</td>											
										</tr>
										<?php $i++;} } } ?>
									</tbody>
							...
							<div class="box-content">
							<h4>COUNTRY BASED</h4>
								<form action="<?php echo WEB_URL_ADMIN ?>b2b/b2bmarkupinsertcountry/1" method="POST" class='form-horizontal form-validate' id="bb">
                                 <div class="control-group">
										<label for="textfield" class="control-label">Agent Name</label>
										<div class="controls">
											<select name="agent" id="agent" class="input-xlarge" data-rule-required="true">
							...
							<div class="box-content nopadding">
								<table class="table table-hover table-nomargin table-bordered dataTable-columnfilter dataTable">
									...
									<tbody>
										<?php $i =1; if(isset($b2b_markup_s)) { if($b2b_markup_s != '') { foreach($b2b_markup_s as $key => $list) { ?>
										<tr>
											<td><?php echo $i; ?></td>
											<td><?php echo $list->api; ?></td>
                                            	<td><?php echo $list->agent_id; ?></td>
