...
//echo $fac;
			...
			$select = "SELECT SQL_CALC_FOUND_ROWS *, MIN(t.total_cost) as low_cost FROM api_hotel_detail_t t, api_permanent p, api_permanent_facility_hotel f WHERE t.hotel_code = p.hotel_code and t.hotel_code = f.hotel_code AND t.session_id = '$ses_id' $where AND $facility_where GROUP BY t.hotel_code $order";
		...
		{
			$select2 = "select MIN(low_cost) as minVal, MAX(low_cost) as maxVal from (
				SELECT MIN(t.total_cost) as low_cost FROM api_hotel_detail_t t, api_permanent p, api_permanent_facility_hotel f WHERE t.hotel_code = p.hotel_code and t.hotel_code = f.hotel_code AND t.session_id = '$ses_id' $where AND $facility_where group by  t.hotel_code ) as tab";
