...
class Asiantravel_Model extends CI_Model {
	...
	function set_crediential($api)
	{
		...
		else
		{
			...
			$this->email = '';
			$this->password = $api_info->password;
	...
	function set_variables($api)
	{
		...
		//	print_r($city_info);exit;
			$this->city_code = $city_info->Asiantravel_Citycode;
			$this->country_code = $city_info->Asiantravel_Countrycode;
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <soap:Header>
    <SOAPHeaderAuthentication MyAttribute="" xmlns="http://instantroom.com/">
      <UserName>'.$this->client_id.'</UserName>
  ...
  <soap:Body>
    <SearchHotelByHotelIdV2 xmlns="http://instantroom.com/">
      ...
      <InstantConfirmationOnly>false</InstantConfirmationOnly>
    </SearchHotelByHotelIdV2>
  </soap:Body>
</soap:Envelope>';


	$soapUrl = $this->post_url.'?op=SearchHotelByHotelIdV2';	
	//echo $request;
				...
				$markup = $this->Hotel_Model->get_b2c_markup($this->api_auto_id);
					...
					foreach($RoomOccupancyRate as $va13)
					{
						...
						$Rateva =$Rate->item(0)->nodeValue;
								if($markup!='0')
								{
								$room_rate_was =  ((($Rateva/100)*$markup)+$Rateva);
								}
								else
								{
								$room_rate_was = $Rateva;
								}	
						$RoomOccupancyRate_Val[] = array('OccupancyId'=>$Occupancyva,'occrate_Dateval'=>$Dateva,'occrate_Rateval'=> number_format($room_rate_was, 2, '.', ''));
				...
				//	print_r($RoomOccupancyRate_Val);
					...
					foreach($RoomOccupancy as $va1l1)
					{
						...
						$Rateval =$Rate->item(0)->nodeValue;
											...
											if($markup!='0')
								{
								$room_rate_w =  ((($Rateval/100)*$markup)+$Rateval);
								}
								else
								{
								$room_rate_w = $Rateval;
								}	
						$RoomOccupancy_Val[] = array('OccupancyId'=>$roorm_OccupancyIdval,'Rateval'=> number_format($room_rate_w, 2, '.', ''),'Rateval_org'=> $Rateval);
					}
...
//print_r($PromotionExceptionDatesvall);
					...
					for($c=0;$c<count($RoomOccupancySeq_Val);$c++)
					{
						...
						for($cv=0;$cv<count($RoomOccupancy_Val);$cv++)
						{
							...
							{
								//echo $RoomOccupancy_Val[$cv]['Rateval'];
								
							//	echo $room_rate_w;exit;
								$new_rate = array('OccupancyId'=>$RoomOccupancySeq_Val[$c]['OccupancyId'],'NoAdult'=>$RoomOccupancySeq_Val[$c]['NoAdult'],'NoChild'=>$RoomOccupancySeq_Val[$c]['NoChild'],'Seq'=>$RoomOccupancySeq_Val[$c]['Seq'],'Rate'=>$RoomOccupancy_Val[$cv]['Rateval'],'Rate_org'=>$RoomOccupancy_Val[$cv]['Rateval_org']);
							//	$markup = $this->Hotel_Model->get_b2c_markup($this->api_auto_id);
						...
						$total_cost_org = $total_cost_org + $RoomOccupancySeq_Val[$c]['Rate_org'];
		...
		else
		{
			...
			$hotel_name =  $this->Hotel_Model->get_permanent_details_v3_asiantravel_hotel_name($HotelCodeval);		
...
//echo $request;
	...
	$soapUrl = $this->post_url.'?op=SearchHotelsByDestV2';	
...
//echo $response;exit;
	...
	//	echo $this->api_auto_id;
		//echo 'sadas';echo $markup;exit;
					if($markup!='0')
					{
						$AvgPriceval_ = ((($AvgPriceval/100)*$markup)+$AvgPriceval);
					}
					else
					{
						$AvgPriceval_ = $AvgPriceval;
					}
					$mapped_id = $this->get_mapped_id($itemCode);
	...
	function get_mapped_id($hotel_id)
	{
		...
		$this->db->where('Asiantravel_Hotelcode',$hotel_id);
	...
	{
		$this->set_crediential('Asiantravel');
		
$soapUrl = $this->post_url."?op=RetrieveHotelInformationV2"; // asmx URL of WSDL
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <soap:Body>
    <BookHotel xmlns="http://instantroom.com/">
      ...
      <TotalRoomRate>'.$result->org_amt.'</TotalRoomRate>
	   <Availability>'.$bookingstatus.'</Availability>
      <AgentRefNo>'.$parent_pnr_no.'</AgentRefNo>
    </BookHotel>
  </soap:Body>
</soap:Envelope>';
//echo $request;
	$soapUrl = $this->post_url.'?op=BookHotel';	
...
//echo 'sada'.$response;exit;
		...
		if(!empty($response))
		{
				...
				$Cityval='';
				$ErrorMessageval='';	
			$ErrorMessage = $dom2->getElementsByTagName("ErrorMessage");
			foreach($ErrorMessage as $Error)
			{
						$ErrorMessageval=$ErrorMessage->item(0)->nodeValue;
			}
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <soap:Body>
    <RetreiveBooking xmlns="http://instantroom.com/">
      <ReferenceNo>'.$booking_no.'</ReferenceNo>
    </RetreiveBooking>
  </soap:Body>
</soap:Envelope>';
	
	$soapUrl = $this->post_url.'?op=RetreiveBooking';	
	$response = $this->curl($soapUrl,$request);
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <soap:Body>
	...
	$soapUrl = $this->post_url.'?op=CancelBooking';	
	...
	function get_city_code($city)
	{
		...
		$this->db->where('Global_Citycode',$city);
		$query = $this->db->get();
