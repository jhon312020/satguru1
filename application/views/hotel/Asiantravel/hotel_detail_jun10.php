<?php
	...
	$result= $this->Asiantravel_Model->get_hotel_details_xml($id);		
		$api = $api;
		...
		if(isset($result['h_images'][0]['ImageFileNameval']))
		{
		$image=$result['h_images'][0]['ImageFileNameval'];
		}
...
<div class="padding10 part985">
  ...
  <div ></div>
                        ...
                        <div class=""><?php echo $hotel_details->starrating; ?><?php /*?><img src="<?php echo base_url().'assets/images/dummy/star-active'.$hotel_details-> 	starrating.'.png'; ?>" style="border: none;"/><?php */?></div>
                        <div class="clr_space"></div>
                        <div style="width:444px; float:left; color:#333; font-size:11px; line-height:15px;  margin-top:-7px; margin-bottom:-7px;">
                    <p><!--<b><?php //echo preg_replace("/[^a-z0-9_-]/i", " ", $hotel_details->HotelDesc); ?></b> <br>--><b><?php echo preg_replace("/[^a-z0-9_-]/i", " ",  substr($result['HotelDesc'],0,200)); ?></b></p><div class="clr"></div>
                </div>
                        <div class="clr_space"></div>
                        <?php  
...
<?php
           ...
           <div class="row-fluid">
              ...
              <div class="span12">
                  <div class=""><br /><?php 
					...
					for($ssd=0;$ssd<count($result['HotelFacilityVas']);$ssd++)
					{
							 echo  '&bull; '.$result['HotelFacilityVas'][$ssd].''.'<br> ';
	...
	for($im=0;$im<count($result['h_images']);$im++)
	{
		?>
...
margin-top: 10px;
text-align: center;">
      <a href="<?php  echo $result['h_images'][$im]['ImageFileNameval']; ?>" rel="lightbox"   title="<?php echo $result['h_images'][$im]['ImageNameval']; ?>" > <img width="230" height="165" src="<?php echo $result['h_images'][$im]['ImageFileNameval']; ?>" /> </a>
      <br /><span style="color:#039"><strong><?php echo $result['h_images'][$im]['ImageNameval']; ?></strong></span>
      </div>
        <?php
	}
	?>
                    ...
                    <div class="tabcontent hide" id="cont-5-1"> 
                        <div class="padding10">
                            <?php 
                                      if(isset($result['HotelDesc']))
                                {
                                echo '</br>';
							
                                    echo htmlspecialchars($result['HotelDesc']);
...
function initialize() {
  var myLatlng = new google.maps.LatLng(<?php print $result['Latitude']; ?>,<?php print $result['Longitude']; ?>);
  var mapOptions = {
    zoom: 14,
    center: myLatlng
  };
...
<script>
	...
	$.ajax({
			  type: 'POST',
			  url: api_url+'/api/call_api/Asiantravel/<?php echo $id; ?>',
			  data: '',
			  async: true,
			  dataType: 'json',
			  beforeSend:function(){
